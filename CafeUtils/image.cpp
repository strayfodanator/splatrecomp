...
Image Image::ParseImage(const uint8_t* data, size_t size)
{
    ...
    // Both plain ELF and RPX start with the ELF magic
    if (data[0] == ELFMAG0 && data[1] == ELFMAG1 && data[2] == ELFMAG2 && data[3] == ELFMAG3)
    {
        const auto* hdr = reinterpret_cast<const elf32_hdr*>(data);
        // RPX uses e_type = 0xFE01 (big-endian on disk)
        uint16_t e_type = ByteSwap(hdr->e_type);
        if (e_type == 0xFE01)
            return RpxLoadImage(data, size);
        else
...
Image ElfLoadImage(const uint8_t* data, size_t size)
{
    ...
    assert(header->e_ident[EI_DATA] == 2); // Must be big-endian
    ...
    for (size_t i = 0; i < numSections; i++)
    {
        ...
        const auto sectionSize = ByteSwap(section.sh_size);

        image.Map(name ? name : "", rva, sectionSize, flags, image.data.get() + ByteSwap(section.sh_offset));
    }
